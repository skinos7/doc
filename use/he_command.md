...
+ 命令的返回类型
	+ **字符串**              用于表示一个单一的信息(被调用的函数返回talk_t类型的字符串), ***以换行符结尾后并以#加一个空格在新行提示***
	+ **ttrue** 或 **tfalse** ttrue通常用于表示操作成功, tfalse表示操作失败, ***以换行符结尾后并以#加一个空格在新行提示***
	+ **{JSON}**              用于表示一些复杂的信息, 所含多种信息, 以{开头并以}结尾的JSON对象(被调用的函数返回talk_t类型的json结构), ***以换行符结尾后并以#加一个空格在新行提示***
...
+ 当调用组件接口带多参数时
    ...
    ```
+ 当调用组件接口返回JSON时, 可以要求只返回JSON中指定的属性值
    ```shell
    组件名称.接口名称:属性路径
    ```
    ```shell
    # ifname@lte2.status  // 调用ifname@lte2的status接口, 即查询系统中第二个LTE的状态
    {
        "mode":"dhcpc",
        "ifname":"ifname@lte2",
        "netdev":"usb1",
    ...
    # ifname@lte2.status:ip                    // 调用ifname@lte2的status接口, 即查询系统中第二个LTE的状态中的ip地址
    10.33.13.103
    # ifname@lte2.status:operator_advise      // 调用ifname@lte2的status接口, 即查询系统中第二个LTE的状态中的有关运营商的建议配置
    {
        "name":"中国联通",
        "dial":"*99#",
        "apn":"3gnet"
    }
    # ifname@lte2.status:operator_advise/apn  // 调用ifname@lte2的status接口, 即查询系统中第二个LTE的状态中的有关运营商的建议的APN的配置
    3gnet
    ```
...
## 常用的HE指令
以下介绍常用的HE指令， 如查询4G信息， GPS信息及重置设备等指令， 更多指令可参看各个组件的接口文档
