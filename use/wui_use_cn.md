...
SkinOS为一个基于Linux的操作系统, 运行于 **网关** 或 **路由器** 等终端产品中, 提供以下两种方式管理SkinOS系统   
-  **网页管理界面**, 使用浏览器登陆网关的WEB界面对其管理, 此文档将介绍此方式管理网关   
-  **终端管理命令行**, 使用Telnet或SSH或串口工具登陆网关使用命令行管理, 使用说明见 **[<<终端管理命令行使用介绍>>](./tui_use_cn.md)**   

---
## 如何登录网页管理界面   

### 电脑通过有线或无线连接网关后打开浏览器    
- 在浏览器地址栏输入 **192.168.8.1** 后回车进入网关 **网页管理界面** 的 **登录界面**   
- 输入用户名, 默认的用户名是 **admin**   
...
**4G5G猫**: 提供4G/5G猫的功能, 完成与4G/5G网络连接但不获取IP地址, 需要其它的客户端通过DHCP协议获取4G/5G的IP地址   
...
点击 **红框1** **系统** 下 **红框2** **设备管理** 后点选 **红框3** 中对应的工作模式即可切换, 切换工作模式需要 **重启系统**   
![avatar](./mode_cn.png)   
