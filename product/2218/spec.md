...
| **网关管理**| 备份及导入配置，本地升级, 在线升级, 局域网批量升级<br>LOGO上传，WEB管理界面定制上传，定制默认配置，*恢复出厂也可保留*<br>英文切换, *恢复出厂也可保留*<br>可安装卸载FPK软件包, 可恢复原始固件状态<br>云平台远程管理<br>Telnet/SSH连入网关<br>串口命令行管`理（CLI）<br>指示灯开关<br>本地及远程日志<br>**升级失败可恢复**（不死Bootloader）<br>可定制普通用户及超级用户界面区分管理, **锁定指定配置**, **锁定升级**, **锁定重启**, **锁定恢复出厂及锁定配置导入导出**等功能<br>**提供x86/aarch64(arm)平台的网关管理工具包及OTA升级工具**<br>支持专有工具实现 **批量升级**, **批量测试** |
