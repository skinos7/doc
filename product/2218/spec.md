...
###### 2218为一款小巧的双网口工业用LTE路由器, 此路由器含有
 - **2G/3G/4G全网通频段接入**(*4G双天线*), **两个百兆以太网口(RJ45)**, **双串口**, *串口1为RS232, 串口2为RS485(可定制为RS232)*
 - **工业用大SIM卡槽**
 - **无线2.4G双天线**(*300Mbit/s*)
 - **凤凰头供电**
 - **GPS定位功能**
...
| **LAN口** | RJ45 LAN接口(10/100Mbps自适应), 自带LINK指定灯  |
| **串口1** | 8PIN凤凰头中的两PIN为串口1接口, *默认**RS232**,可定制提供TTL电平串口*  |
| **串口2** | 8PIN凤凰头中的两PIN为串口2接口, *默认**RS485串口**, 可定制为RS232电平串口* |
| **凤凰头供电口** | 8PIN凤凰头中的两PIN为电源供电口(**供电范围为7-24V**)  |
| **WIFI天线接口(2个)** | WIFI2.4G SMA天线接口*2(外螺纹+孔)  |
| **3G/4G天线接口(2个)** | 3G/4G SMA天线接口*2(外螺纹+孔) |
| **GPS天线接口** | GPS SMA天线接口(外螺纹+孔) |
| **SIM卡槽** | 抽屉式大SIM卡槽 |
| **接键(1个)** | 复位按键 |
| **指示灯(4个)** | **系统灯(蓝)**:刚上电时常亮，启动后但未连接慢闪，正在拨号时快闪， 连接网络成功后常亮<br>**信号灯**: 4G信号弱时亮红, 4G信号强时亮绿<br>**WAN**: WAN口LINK灯<br>**LAN**: LAN口LINK灯 |
| **凤凰头接口** | 3.5mm间距共8PIN<br>包含串口1, 串口2, 电源供电口, 3.3V电源及GND引脚  |
...
|  功能   | 描述  |
| :-----:| :---- |
| **联网方式** | **单连接上网**： 3G/4G上网，有线上网，无线上网，桥接，中继，无线互联网, 3G/4G透传(Modem)模式<br>**混合(多WAN)上网**：<br>2G/3G/4G与以太网有线上网同时工作, 备份及负载均衡带宽叠加<br>2G/3G/4G与无线WIFI上网同时工作, 备份及负载均衡带宽叠加<br>无线WIFI上网与以太网有线上网同时工作, 备份及负载均衡带宽叠加<br>**组合上网模式**: 3G/4G上网+桥接， 3G/4G上网+有线接内网, 3G/4G上网+无线接内网, 其它复杂定制组合 |
...
| **虚拟专用网**| 支持L2TP客户端(*支持隧道密码*)<br>PPTP客户端(*支持MPPE*)<br>GRE隧道<br>Open威批恩客户端(*支持预共享密钥,证书*)<br>IPSEC<br>ZeroTier<br>WireGuard |
...
| **设备管理**| 备份及导入配置，本地升级, 在线升级, 局域网批量升级<br>LOGO上传，WEB管理界面定制上传，定制默认配置，*恢复出厂也可保留*<br>英文切换, *恢复出厂也可保留*<br>可安装卸载FPK软件包, 可恢复原始固件状态<br>云平台远程管理<br>Telnet/SSH连入设备<br>串口命令行管`理（CLI）<br>指示灯开关<br>本地及远程日志<br>**升级失败可恢复**（不死Bootloader）<br>可定制普通用户及超级用户界面区分管理, **锁定指定配置**, **锁定升级**, **锁定重启**, **锁定恢复出厂及锁定配置导入导出**等功能<br>**提供x86/aarch64(arm)平台的设备管理工具包及OTA升级工具**<br>支持专有工具实现 **批量升级**, **批量测试** |
...
- -/+为给路由器供电的接线引脚, 电源供电口, 支持输入7V至24V直流电
- B为串口2即RS485的模式下为B
- A为串口2即RS485的模式下为A
- G为GND接地
    > 使用串口时可以从这个管脚取地
...
###### LAN口: **为带有LINK灯的为有线LAN网口**, LAN口LINK灯位于路由器正面
- 为LAN口, 用于连接其它有线设备, 为其提供上网服务

###### WAN口: **为带有LINK灯的为有线WAN/LAN网口**, WAN口LINK灯位于路由器正面
